        ...
        <section id="my-properties" class="max-w-7xl mx-auto px-6 mb-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden property-card">
                    <div class="relative">
                        <img src="{{ $propertie->photos ? asset('storage/' . $propertie->photos[0]) : '/api/placeholder/400/250' }}"
                            alt="{{ $propertie->name }}" class="w-full h-48 object-cover">

                        <div class="absolute top-3 left-3 bg-green-500 text-white text-xs font-medium px-2 py-1 rounded-full">
                            {{ $propertie->status ?? 'Available' }}
                        </div>
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center text-gray-600 text-sm mb-2">
                            ...
                            {{ $propertie->city }}, {{ $propertie->country }}
                        </div>
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm text-gray-700 ml-1">
                                    {{ number_format($propertie->rating ?? 4.5, 1) }} ({{ $propertie->reviews_count ?? 10 }} reviews)
                                </span>
                            </div>
                            <span class="text-blue-600 font-bold">€{{ number_format($propertie->base_price, 2) }}/night</span>
                        </div>

                        <div class="flex flex-wrap gap-2 mb-4">
                            <span class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
                                {{ $propertie->bedrooms }} Bedrooms
                            </span>
                            <span class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
                                {{ $propertie->bathrooms }} Baths
                            </span>
                            <span class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">
                                {{ $propertie->max_guests }} Guests
                            </span>

                        </div>
                        <div class="flex space-x-2">
                            <a href="{{ route('properties.edit', $propertie->id) }}" class="flex-1 bg-blue-100 hover:bg-blue-200 text-blue-600 text-center py-2 rounded-lg transition">
                                Edit
                            </a>
                              <form action="{{ route('properties.destroy', $propertie->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex-1 bg-red-100 hover:bg-red-200 text-red-600 py-2 rounded-lg transition">
                                    Delete
                                </button>
                            </form>
                             </div>
