            ...
            <div id="reservationModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
                <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 transform transition-all duration-300 scale-95 opacity-0" id="modalContent">
                    <div class="flex justify-between items-center mb-4 border-b pb-4">
                        <h3 class="text-xl font-bold text-gray-800" id="modalPropertyName">Property Name</h3>
                    ...
                    <form id="reservationForm">
                        ...
                        <div class="mb-4">
                            <label for="checkout" class="block text-gray-700 font-medium mb-2">Check-out Date</label>
                        ...
                        <div class="mt-6 border-t pt-4">
                            <div class="flex justify-between items-center mb-4">
                                <span class="text-gray-700">Total nights: <span id="totalNights">0</span></span>
                                <span class="text-gray-700">Estimated total: <span id="estimatedTotal" class="font-bold">$0.00</span></span>
                            </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchBookedDates() {
        fetch(`/api/booked-dates/${propertyId}`)
            .then(response => response.json())
            .then(data => {
                let bookedDates = data.booked_dates.map(date => new Date(date).toISOString().split('T')[0]);

                checkinInput.addEventListener("change", function () {
                    let selectedCheckin = checkinInput.value;
                    checkoutInput.min = new Date(new Date(selectedCheckin).setDate(new Date(selectedCheckin).getDate() + 1)).toISOString().split('T')[0];
                ...
                checkoutInput.addEventListener("change", function () {
                    if (bookedDates.includes(checkoutInput.value)) {
                        alert("This date is already booked. Please choose another date.");
                        checkoutInput.value = "";
                    }
                });
    ...
    function fetchBookedDates() {
        fetch(`/api/booked-dates/${propertyId}`)
            ...
            .then(data => {
                ...
                // ✅ تعطيل التواريخ المحجوزة في `checkin`
                checkinInput.addEventListener("input", function () {
                    if (bookedDates.includes(checkinInput.value)) {
                        alert("This date is already booked. Please choose another date.");
                        checkinInput.value = "";
                    }
                });

                // ✅ تعطيل التواريخ المحجوزة في `checkout`
                ...
                // ✅ منع اختيار التواريخ المحجوزة في التقويم (HTML5)
                checkinInput.addEventListener("click", function () {
                    disableBookedDates(checkinInput, bookedDates);
                });

                checkoutInput.addEventListener("click", function () {
                    disableBookedDates(checkoutInput, bookedDates);
                });

                function disableBookedDates(input, bookedDates) {
                    let datePicker = input;
                    datePicker.addEventListener("focus", function () {
                        let dateList = bookedDates.map(date => `"${date}"`).join(", ");
                        datePicker.setAttribute("onkeydown", "return false"); // منع إدخال يدوي
                        datePicker.setAttribute("onfocus", `this.showPicker();`); // عرض التقويم عند النقر
                        datePicker.setAttribute("oninput", `if ([${dateList}].includes(this.value)) this.value = '';`);
                    });
                }
            });
    }
