...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch(`/api/booked-dates/${propertyId}`)
        ...
        .then(data => {
            ...
            // Initialiser flatpickr pour le champ checkin et checkout
            flatpickr(checkinInput, {
                minDate: "today", // Limiter à aujourd'hui ou après
...
function disableBookedDates(input, bookedDates) {
    input.addEventListener("focus", function () {
        ...
        bookedDates.forEach(date => {
            ...
            option.value = date;
            option.disabled = true; // Attempt to disable options (may not work in all browsers)
            datalist.appendChild(option);
        });

        document.body.appendChild(datalist);
        input.setAttribute("list", "disabled-dates");
    });
