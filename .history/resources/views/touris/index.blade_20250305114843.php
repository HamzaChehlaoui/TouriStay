    ...
    <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 transform transition-all duration-300 scale-95 opacity-0" id="modalContent">
        ...
        <form id="reservationForm">
            ...
            <div class="mt-6 border-t pt-4">
                ...
                <button type="button" id="payButton" class="w-full bg-green-600 text-white py-3 rounded-lg font-medium hover:bg-green-700 transition-colors flex items-center justify-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a4 4 0 00-8 0v2H5v6h14V9h-2z"></path>
                    </svg>
                    Pay New
                </button>
...
<script>
    ...
    function setupDatePickers(propertyId) {
        // جلب التواريخ المحجوزة من السيرفر
        fetch(`/api/booked-dates/${propertyId}`)
            .then(response => response.json())
            .then(data => {
                const bookedDates = data.booked_dates.map(date => new Date(date).toISOString().split('T')[0]); // تحويل التاريخ إلى تنسيق مناسب

                // تهيئة Date Picker الخاص بتاريخ الوصول (Check-in)
                flatpickr("#checkin", {
                    minDate: "today", // تعطيل التواريخ الماضية
                    disable: bookedDates, // تعطيل التواريخ المحجوزة
                    dateFormat: "Y-m-d",
                    onDayCreate: function(dObj, dStr, fp, dayElem) {
                        let date = new Date(dayElem.dateObj).toISOString().split('T')[0];
                        if (bookedDates.includes(date)) {
                            // إخفاء التواريخ المحجوزة من واجهة المستخدم
                            dayElem.classList.add('flatpickr-disabled');
                            dayElem.classList.add('flatpickr-day-booked');
                            dayElem.style.visibility = 'hidden';
                            dayElem.style.display = 'none';
                    ...
                    onChange: function(selectedDates, dateStr, instance) {
                        // تحديث Date Picker الخاص بتاريخ المغادرة (Checkout) بحيث لا يمكن اختيار تواريخ محجوزة بعد تاريخ الوصول
                        ...
                        // إعادة حساب الإجمالي وعدد الليالي
                        calculateTotalAndNights();
                    }
                });

                // تهيئة Date Picker الخاص بتاريخ المغادرة (Checkout)
                flatpickr("#checkout", {
                    minDate: "today", // تعطيل التواريخ الماضية
                    disable: bookedDates, // تعطيل التواريخ المحجوزة
                    dateFormat: "Y-m-d",
    ...
    // تعديل الدالة openReservationModal لتضمين setupDatePickers
    ...
    window.openReservationModal = function(propertyId, propertyName, propertyPrice) {
        ...
        // إعداد التقويمات الخاصة بالتواريخ المحجوزة
